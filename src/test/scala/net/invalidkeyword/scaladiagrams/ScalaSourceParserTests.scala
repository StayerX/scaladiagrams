...
class ScalaSourceParserTests extends FlatSpec with Matchers {
  ...
  it should "not crash and fail to parse accent random text" in {
    val result = ScalaSourceParser.run("val input: String = \"aáeéiíoóöőuúüű AÁEÉIÍOÓÖŐUÚÜŰ\"")
    result.successful should be(true)
    ScalaSourceParser.filter(result.get) should be(List())
  }
  ...
  it should " parse a value assign and return the class" in {
    ...
    result.successful should be(true)
    ScalaSourceParser.filter(result.get) should be(List(VALCLASS("Parser", List(RELATED("customClassName")))))
  }
